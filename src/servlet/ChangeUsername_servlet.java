...
public class ChangeUsername_servlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(req.getIdentity()==1)
			update=String.format("UPDATE student SET username='%s' WHERE id='%s'",req.getChangeItem(), req.getId());
		else
			update=String.format("UPDATE teacher SET username='%s' WHERE id='%s'", req.getChangeItem(),req.getId());
		try {
