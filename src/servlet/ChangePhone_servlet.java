...
/**
 * Servlet implementation class ChangePhone_servlet
 */
@WebServlet("/user/changePhone")
public class ChangePhone_servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChangePhone_servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(req.getIdentity()==1)
			update=String.format("UPDATE student SET phone='%s' WHERE id='%s'",req.getChangeItem(), req.getId());
		else
			update=String.format("UPDATE teacher SET phone='%s' WHERE id='%s'",req.getChangeItem(), req.getId());
		try {
